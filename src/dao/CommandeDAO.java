...
public class CommandeDAO {
    ...
    public int creerCommande(int clientId, String modeRecuperation, String adresseLivraison) throws SQLException {
        String sql = "INSERT INTO commandes (client_id, mode_recuperation, adresse_livraison, etat, total) VALUES (?, ?, ?, 'non traitée', 0.00)";
    ...
    // ===== NOUVELLES MÉTHODES POUR LA GESTION DU TOTAL =====

    /**
     * Met à jour le total d'une commande en recalculant depuis les détails
     * @param commandeId ID de la commande
     * @return true si la mise à jour a réussi
     */
    public boolean updateTotalCommande(int commandeId) throws SQLException {
        String sql = "UPDATE commandes SET total = (" +
                "SELECT COALESCE(SUM(dc.quantite * dc.prix_unitaire), 0) " +
    ...
    /**
     * Met à jour le total d'une commande avec une valeur spécifique
     * @param commandeId ID de la commande
     * @param nouveauTotal Nouveau montant total
     * @return true si la mise à jour a réussi
     */
    public boolean updateTotalCommande(int commandeId, double nouveauTotal) throws SQLException {
     ...
     */
    public double getTotalCommande(int commandeId) throws SQLException {
        String sql = "SELECT total FROM commandes WHERE id = ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, commandeId);
            ResultSet rs = ps.executeQuery();
